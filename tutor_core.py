...
COMPLETION_FEEDBACK_SYSTEM = """
你是数学游戏导师兼学习教练。学生已经完成当前题，position、strategy、pattern 三个维度累计均已正确。

请生成当前题的结题反馈，必须按顺序包含三件事：
1. 简短恭喜学生答对本题；
2. 用你自己的话讲清这道题的 math_logic 核心思想，不要照抄材料，不要继续追问；
3. 基于学生在本题中的所有回复，给出具体的 metacognitive feedback：他掌握好的思维动作是什么，薄弱处或依赖引导的地方是什么，接下来应保持或改进什么。

要求：
- 只评价当前题，不要出延伸题，不要提下一题内容；
- 不要完整照搬 answer、strategy、math_logic；
- 不要批评人格，只评价可观察的学习表现；
- 如果学生表现很好，可以说保持现状，不要硬找缺点；
- 输出 2～4 个自然段，总字数控制在 300 字以内。
"""
...
def _completion_feedback_fallback(meta: pd.Series, stats: dict | None = None) -> str:
    inc = int((stats or {}).get("incorrect_count") or 0)
    ml = _clip(meta.get("math_logic", ""), 420)
    if inc > 0:
        feedback = "这题中间有过一些调整，后面继续保持先验证小局面、再总结规律的节奏，会更稳。"
    else:
        feedback = "这题推进得比较顺，后面可以继续保持先找控局规律、再联系总数判断先后手的习惯。"
    return (
        "答得很好，这道题你已经把关键结论、拿法和背后的规律都理清楚了。\n\n"
        f"这题的核心思想是：{ml}\n\n"
        f"{feedback}"
    )


def generate_completion_feedback_reply(
    meta: pd.Series,
    question_messages: list | None,
    stats: dict | None = None,
) -> tuple[str, dict]:
    hist_lines: list[str] = []
    for m in (question_messages or [])[-30:]:
    ...
    payload = (
        ...
        f"【math_logic】\n{_clip(meta.get('math_logic', ''), 6000)}\n\n"
        f"【strategy】\n{_clip(meta.get('strategy', ''), 6000)}\n\n"
        f"【answer 参考】\n{_clip(meta.get('answer', ''), 1200)}\n\n"
        "【当前题累计状态】\n"
        f"position={s.get('accum_position', '')}; "
        f"strategy={s.get('accum_strategy', '')}; "
        f"pattern={s.get('accum_pattern', '')}; "
        f"错误次数={s.get('incorrect_count', 0)}; "
        f"行为序列={list(s.get('behavior_sequence') or [])}\n\n"
        f"【当前题对话记录】\n{history}\n"
    )
    text, raw = llm_text_reply(
        COMPLETION_FEEDBACK_SYSTEM.strip(),
        payload,
        temperature=0.3,
        timeout_sec=180,
        retries=3,
    ...
    if text and text.strip():
        return text.strip(), raw
    return _completion_feedback_fallback(meta, stats), raw
